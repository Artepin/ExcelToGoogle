    ...
    for row in range(len(excel_data_raw)):
        ...
        while(len(data_google) != row+1):
            ...
            while True:
                ...
                rng = (el.columnLetter(int(temp + 1)) + str(excel_data_raw[row])+":"+ el.columnLetter(int(half + temp)) + str(excel_data_raw[row]))
                try:
                    check = ss.getData(rng,link_id)
                except:
                    break
                if (str(el.columnLetter(int(temp+1)) + str(excel_data_raw[row]))) == (str(el.columnLetter(int(half+temp)) + str(excel_data_raw[row]))):
...
rng = (str(data_google[0]) + str(int(data_google[1])+1) + ":" + str(data_google[0]) + str(int(data_google[1])+rows - int(data_excel[1])))
    ...
    if excel_cell_vals[cell_val] != 'None':
        try:
            if len(google_cell_vals[0][cell_val]) == 0:
                ss.prepare_setValues(str(data_google[0]) + str(int(data_google[1])+cell_val+1), [[excel_cell_vals[cell_val]]])
                continue
